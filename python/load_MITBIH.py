...
def save_wvlt_PCA(PCA, pca_k, family, level):
    with open('Wvlt_' + family + '_' + str(level) + '_PCA_' + str(pca_k) + '.p', 'wb') as f:
        pickle.dump(PCA, f, 2)


def load_wvlt_PCA(pca_k, family, level):
    with open('Wvlt_' + family + '_' + str(level) + '_PCA_' + str(pca_k) + '.p', 'rb') as f:
...
def load_mit_db(DS, winL, winR, do_preprocess, maxRR, use_RR, norm_RR, compute_morph, db_path, reduced_DS, leads_flag):
    ...
    if os.path.isfile(features_labels_name):
        ...
        mit_pickle_name = os.path.join(db_path, 'python_mit')
        if reduced_DS:
        ...
        if use_RR or norm_RR:
            if DS == 'DS1':
                RR = [RRIntervals() for i in range(len(DS1))]
            else:
                RR = [RRIntervals() for i in range(len(DS2))]
...
def load_signal(DS, winL, winR, do_preprocess):
    ...
    lst = os.listdir(os.path.join(pathDB, DB_name))
    lst.sort()
    for file in lst:
    ...
    for r in range(0, len(fRecords)):
        ...
        filename = os.path.join(pathDB, DB_name, fRecords[r])
...
if __name__ == '__main__':
    load_signal('DS1', winL=90, winR=90, do_preprocess=True)