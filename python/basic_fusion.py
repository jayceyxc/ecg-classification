#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# @Time    : 2019-05-31 15:15
# @Author  : yuxuecheng
# @Contact : yuxuecheng@xinluomed.com
# @Site    : 
# @File    : basic_fusion.py
# @Software: PyCharm
# @Description
...
def main():
    ...
    # Load gt labelso
    eval_labels = np.loadtxt('/Users/yuxuecheng/Learn/OpenSource/medicine/ecg-classification/python/mit_db/' + DS + '_labels.csv')
    ...
    predict, prob_ovo_RR_sig = ovo_voting_exp(prob_ovo_RR,
                                              4)  # voting_ovo_w(prob_ovo_RR)           #voting_ovo_raw(prob_ovo_RR)
    predict, prob_ovo_wvl_sig = ovo_voting_exp(prob_ovo_wvl,
                                               4)  # voting_ovo_w(prob_ovo_wvl)          #voting_ovo_raw(prob_ovo_wvl)
    predict, prob_ovo_LBP_sig = ovo_voting_exp(prob_ovo_LBP,
                                               4)  # voting_ovo_w(prob_ovo_HOS_myDesc)   #voting_ovo_raw(prob_ovo_HOS_myDesc)
    predict, prob_ovo_HOS_sig = ovo_voting_exp(prob_ovo_HOS,
                                               4)  # voting_ovo_w(prob_ovo_HOS_myDesc)   #voting_ovo_raw(prob_ovo_HOS_myDesc)
    predict, prob_ovo_MyDescp_sig = ovo_voting_exp(prob_ovo_MyDescp,
                                                   4)  # voting_ovo_w(prob_ovo_HOS_myDesc)   #voting_ovo_raw(prob_ovo_HOS_myDesc)
    ...
    # 5
    probs_ensemble = np.stack(
        (prob_ovo_RR_sig, prob_ovo_wvl_sig, prob_ovo_HOS_sig, prob_ovo_LBP_sig, prob_ovo_MyDescp_sig))
    ...
    write_AAMI_results(perf_measures, results_path + 'fusion/prod_rule_score_Ijk_' + str(
        format(perf_measures.Ijk, '.2f')) + '_' + DS + '.txt')
